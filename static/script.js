        ...
        success: function (response) {
            if (response["result"] === "success") {
                ...
                for (let i = 0; i < posts.length; i++) {
                    let post = posts[i];
                    console.log(post["profile_pic_real"])
                                                  ...
                                                  <nav class="level is-mobile">
                                                      <div class="level-left">
                                                          ...
                                                          <a class="level-item is-sparta" aria-label="star" onclick="toggle_star('${post["_id"]}', 'star')">
                                                              <span class="icon is-small"><i class="fa ${class_star}" aria-hidden="true"></i></span>&nbsp;<span class="star-num">${num2str(post["count_star"])}</span>
                                                          </a>
                                                          <a class="level-item is-sparta" aria-label="thumbsup" onclick="toggle_thumbsup('${post["_id"]}', 'thumbsup')">
                                                              <span class="icon is-small"><i class="fa ${class_thumbsup}" aria-hidden="true"></i></span>&nbsp;<span class="thumbsup-num">${num2str(post["count_thumbsup"])}</span>
                                                          </a>
                                                      </div>
                                                  </nav>
                                              </div>
    ...
    if ($i_like.hasClass("fa-thumbs-up")) {
        $.ajax({
            ...
            success: function (response) {
                ...
                $a_like.find("span.thumbsup-num").text(response["count"]);
            },
        });
    } else {
        $.ajax({
